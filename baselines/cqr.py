"""Deep Ensembles baseline (Lakshminarayanan et al., 2017)."""

import numpy as np
import torch
from spectre.trainer import train_model
from spectre.model import MLP
from ._utils import make_loader, get_probs, compute_metrics


def run_deep_ensembles(Xtr, ytr, Xval, yval, Xte_sets: dict,
                       n_cls: int, device, seed: int = 42,
                       n_ens: int = 5, epochs: int = 30, lr: float = 1e-3) -> dict:
    ltr  = make_loader(Xtr, ytr)
    lval = make_loader(Xval, yval, shuffle=False)
    members = []
    for i in range(n_ens):
        torch.manual_seed(seed + i)
        m = MLP(Xtr.shape[1], n_cls).to(device)
        train_model(m, ltr, epochs=epochs, lr=lr, val_loader=lval, device=device)
        members.append(m)

    pr_in = np.mean([get_probs(m, Xval, device) for m in members], 0)
    conf_in = pr_in.max(1)
    results = {}
    for name, Xte in Xte_sets.items():
        pr_out   = np.mean([get_probs(m, Xte, device) for m in members], 0)
        conf_out = pr_out.max(1)
        results[name] = compute_metrics(-conf_in, -conf_out)
    return results
